...
from src.return_collision_frequencies import * # Project Related Data/Functions
from src.determine_collision import * # Project Related Data/Functions
from src.propagate_electrons import run_electrons # Project Related Data/Functions
...
def return_top_cfs(num_cfs = 1):
    """Returns a pandas.DataFrame of the highest collision frequencies and the energies/processes for which they happen.
    An example output would look like:

    .. table:: Top Collision Frequencies Pandas DataFrame
        :widths: 6 10 6 12

        +-------------+---------------------+-------------+---------------------+
        |             |      collision_type |      energy | collision_frequency |
        +=============+=====================+=============+=====================+
        |           0 |             elastic |       14.00 |        3.170513e+08 |
        +-------------+---------------------+-------------+---------------------+
        |           1 |          ionization |      100.00 |        1.691198e+08 |
        +-------------+---------------------+-------------+---------------------+
        |           2 |                 gp1 |       28.00 |        5.535743e+06 |
        +-------------+---------------------+-------------+---------------------+
        |           . |                   . |           . |                   . |
        +-------------+---------------------+-------------+---------------------+
        |           . |                   . |           . |                   . |
        ...
        |           . |                   . |           . |                   . |
        +-------------+---------------------+-------------+---------------------+
        | num_cfs - 1 | last_collision_type | last_energy | last_collision_freq |
        +-------------+---------------------+-------------+---------------------+
    """

    top_collision_frequencies = pd.DataFrame(columns=["collision_type", "energy", "collision_frequency"])
    ...
    for collision_type in collision_frequencies: # For every process in our copy, we will compute the CF for every energy
        ordered_process_dictionary = OrderedDict(
            sorted(collision_frequencies[collision_type].items(), key=lambda t: t[1], reverse=True))
        energy_of_max_process_cf = list(ordered_process_dictionary)[0]
        top_collision_frequencies = top_collision_frequencies.append(ignore_index=True, other=
            {'collision_type': collision_type, 'energy': energy_of_max_process_cf,
             'collision_frequency': collision_frequencies[collision_type][energy_of_max_process_cf]})

    top_collision_frequencies = top_collision_frequencies.sort_values(by=['collision_frequency'], ascending=False)
    print("top_collision_frequencies")
    print(top_collision_frequencies.reset_index(drop=True).head(num_cfs))
    return top_collision_frequencies.reset_index(drop=True).head(num_cfs)

def return_top_cf_and_neighbors(num_cfs = 1, sum_excitation_states=False, null=False, total=False):
    """Returns a pandas.DataFrame with the highest collision frequency with the energy & process for which it happens as
    ...
    .. table:: CFs at Energy where the Maximum CF Occurs
        ...
        |           0 |             elastic |       14.00 |        3.170513e+08 |
        +-------------+---------------------+-------------+---------------------+
        |           1 |                 gp1 |       14.00 |        3.257905e+05 |
        +-------------+---------------------+-------------+---------------------+
        |           2 |                gp10 |       14.00 |        7.030499e+04 |
        +-------------+---------------------+-------------+---------------------+
        |           3 |                s3p7 |       14.00 |        6.631842e+04 |
        +-------------+---------------------+-------------+---------------------+
        |           . |                 gp5 |       14.00 |        6.478863e+04 |
        +-------------+---------------------+-------------+---------------------+
        |           . |                   . |           . |                   . |
        +-------------+---------------------+-------------+---------------------+
        |           . |                   . |           . |                   . |
        ...
        +-------------+---------------------+-------------+---------------------+
    """
    if sum_excitation_states:
        desired_collision_frequencies = condense_collision_frequency_dictionary(collision_frequencies)
    else:
        desired_collision_frequencies = dict(collision_frequencies)
    top_collision_frequencies = return_top_cfs(num_cfs=num_cfs)
    top_cf_energy = top_collision_frequencies.at[0,'energy']
    top_cf_and_neighbors = pd.DataFrame(columns=["collision_type", "energy", "collision_frequency"])
    for collision_type in desired_collision_frequencies:
        top_cf_and_neighbors = top_cf_and_neighbors.append(ignore_index=True, other=
            {'collision_type': collision_type,
             'energy': top_cf_energy,
             'collision_frequency': desired_collision_frequencies[collision_type][top_cf_energy]})
    top_cf_and_neighbors = top_cf_and_neighbors.sort_values(by=['collision_frequency'], ascending=False)
