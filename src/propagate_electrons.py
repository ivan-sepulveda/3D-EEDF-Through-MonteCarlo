...
import math
import numpy as np
from src.determine_collision import *
from src.return_collision_frequencies import *
from src.initial_conditions import *
import os

def return_iter_evs():
    i_ev = return_interpolated_ev(cs_energy_spacing)
    return i_ev
...
#
class Electron:
    """An simulated electron that may be propagated in a simulated plasma. An Electron() class instance will keep track
    of it's own position, velocity, acceleration, and energy.
    """
    def __init__(self):
    ...
    def update_data_lists(self, x, y, z, t, e_eV, vx, vy, vz, v):
        """Updates the electrons data for position, time, energy (in eV), and velocities.
        ...
        Args:
            ...
            z (float): The second parameter.
            t (float): The second parameter
            e_eV (float): The second parameter
            vx (float): The second parameter
            vy (float): The second parameter
            vz (float): The second parameter
    ...
    def check_for_collision(self):
        """Checks for a collision with an Argon atom or another electron
        """
        ...
        if process != "null":
            ...
            x_ratio = sin(alpha)*cos(beta)*cos(theta) - sin(beta)*sin(phi)*sin(theta)\
                      + sin(theta)*cos(alpha)*cos(beta)*cos(phi)

            y_ratio = sin(alpha)*sin(beta)*cos(theta)+ sin(beta)*sin(theta)*cos(alpha)*cos(phi)\
...
def send_electron(print_status=False):
    ...
    Args:
    	  print_status: If True, prints the coordinates, velocity, accelerations, and  of the electron as it propagates.
